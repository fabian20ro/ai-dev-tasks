...
### 0️⃣ Initialize the Decision Log

Before doing anything else, create an empty `decision-log.md` so future prompts
have a shared place to record irreversible choices.

```text
Use @decision-log.mdc
Create a new decision log for this feature
```

This log grows over time and is referenced by later steps.
...
After saving the file (e.g., `prd-my-feature.md`), check `decision-log.md` for a
new row noting that the feature scope and core stories are locked in. The PRD
includes the latest `context-snapshot.md` so any developer sees the full
background.
...
The task generator reads the decision log before producing parent tasks and writes a delta entry when the sub-tasks are done. You can peek at `decision-log.md` to see the new constraints.
...
Regular summaries generated by `context-summarise.mdc` keep the conversation window manageable so the AI never loses track of earlier decisions.

```text
Use @context-summarise.mdc
```

If a task requires outside knowledge, spin up a scout with:

```text
Use @research-scout.mdc about "[topic]"
```
...
After completing a major parent task, run a quick reflection to ensure the plan still makes sense:

```text
Use @retro-review.mdc
```
...
* **`decision-log.mdc`**: Central log where all irreversible choices are appended.
* **`context-summarise.mdc`**: Periodically compresses the conversation and files into a short context snapshot.
* **`research-scout.mdc`**: Template for short-lived agents that gather information without making decisions.
* **`retro-review.mdc`**: Optional prompt to reflect on progress after major tasks.
